...
if annotFile != '':
	...
	extractString = []
	for c in annotFileColumns.split(','):
		#Subtract one from the column number to become the index
		extractString.append(int(c)-1)
	print('Extracting columns from annotation file: ',  extractString)


	#Read in the homolog File with pandas
	annot_data_table = pd.read_csv(annotFile, delimiter='\t')


	#Rewrite to the subsetted table with columns to extract only
	annot_data_table = annot_data_table.iloc[:, extractString]

	#Ensure each comma has a space after it for viewing in website tables
	annot_data_table = annot_data_table.replace({',': ', '}, regex=True)

	#Rename first column
	annot_data_table = annot_data_table.rename(columns={annot_data_table.columns[0]: "#GeneID" })

	#Write to outfile
	annot_data_table.to_csv(out_annotfile, sep='\t', index=False)
...
print('Will write the extracted homolog information to:\n %s' % out_homologfile, '\n')


#If homolog file is provided, let's parse it
if homologFile != '':
	print('\n' + 'Processing the homolog file: \n %s ' % homologFile +  '\n')
	...
	if homologFileColumns == '':
		print('ERROR: You must provide columns to extract from homolog file!')
		print('Exiting...')
		sys.exit()
	#In case the user put quotation marks, get rid of them
